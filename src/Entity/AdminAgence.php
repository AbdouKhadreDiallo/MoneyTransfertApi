...
class AdminAgence extends User
{
    ...
    /**
     * @ORM\ManyToOne(targetEntity=Agence::class, inversedBy="admin")
     */
    private $agence;

    /**
     * @ORM\OneToMany(targetEntity=Transaction::class, mappedBy="adminSystemAuthorDepot")
     */
    private $transactionDepot;

    /**
     * @ORM\OneToMany(targetEntity=Transaction::class, mappedBy="adminSystemAuthorRetrait")
     */
    ...
    public function addTransactionDepot(Transaction $transactionDepot): self
    {
        if (!$this->transactionDepot->contains($transactionDepot)) {
            ...
            $transactionDepot->setAdminSystemAuthorDepot($this);
        }
    ...
    public function removeTransactionDepot(Transaction $transactionDepot): self
    {
        if ($this->transactionDepot->removeElement($transactionDepot)) {
            ...
            if ($transactionDepot->getAdminSystemAuthorDepot() === $this) {
                $transactionDepot->setAdminSystemAuthorDepot(null);
            }
        }
    ...
    public function addTransactionRetrait(Transaction $transactionRetrait): self
    {
        if (!$this->transactionRetrait->contains($transactionRetrait)) {
            ...
            $transactionRetrait->setAdminSystemAuthorRetrait($this);
        }
    ...
    public function removeTransactionRetrait(Transaction $transactionRetrait): self
    {
        if ($this->transactionRetrait->removeElement($transactionRetrait)) {
            // set the owning side to null (unless already changed)
            if ($transactionRetrait->getAdminSystemAuthorRetrait() === $this) {
                $transactionRetrait->setAdminSystemAuthorRetrait(null);
            }
        }
